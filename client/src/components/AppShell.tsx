...
function NavLink({
  ...
  return (
    ...
    >
      {active && (
        <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r-full bg-[#5b8def]" />
      )}
  ...
  return (
    <div className="h-full flex flex-col py-4 px-3 gap-1 overflow-y-auto custom-scrollbar">
      ...
      <div className="px-3 py-2">
        <NavLink item={primaryNav[0]} collapsed={collapsed} />
      </div>

      <div className="px-3 pt-3 pb-1">
        {!collapsed && (
          <div className="text-[10px] font-semibold uppercase tracking-wider text-[#5a5f68] px-3 mb-1.5">
            Resume
          </div>
        )}
      </div>
      <div className="px-3 flex flex-col gap-0.5">
        {resumeGroup.map((item) => (
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between px-4 py-4 border-b border-[#1f1f26]">
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-2 rounded-lg text-[#8a8f98] hover:text-[#f5f5f7] hover:bg-[#101013] transition-colors duration-150"
        >
  ...
  return (
    <div className={`${densityClass} h-screen w-screen flex flex-row bg-[#0a0a0b] text-[#f5f5f7] overflow-hidden`}>
      <motion.aside
        ...
        className="hidden md:flex flex-shrink-0 h-full border-r border-[#1f1f26] bg-[#101013] overflow-hidden"
      >
        ...
        <header className="flex-shrink-0 flex items-center justify-between px-4 sm:px-6 h-14 md:h-16 border-b border-[#1f1f26] bg-[#101013]">
          <div className="flex items-center gap-2">
            <button
              ...
              className="md:hidden p-2 rounded-lg text-[#8a8f98] hover:text-[#f5f5f7] hover:bg-[#141416] transition-colors duration-150"
            >
            ...
            <button
              ...
              className="hidden md:flex p-2 rounded-lg text-[#8a8f98] hover:text-[#f5f5f7] hover:bg-[#141416] transition-colors duration-150"
            >
              {sidebarCollapsed ? (
        ...
        <nav className="md:hidden flex-shrink-0 border-t border-[#1f1f26] bg-[#101013] safe-bottom">
          <div className="flex items-center justify-between px-2">
            ...
            <button
              type="button"
              onClick={toggleMobileDrawer}
              className="flex flex-col items-center justify-center gap-1 px-2 py-2 rounded-lg transition-colors duration-150 flex-1 min-w-0 text-[#6b7280] hover:text-[#f5f5f7]"
            >
        ...
        {mobileDrawerOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ x: "-100%" }}
              transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
              className="md:hidden fixed inset-y-0 left-0 w-[280px] max-w-[85vw] bg-[#101013] border-r border-[#1f1f26] z-50 overflow-hidden"
            >
